...
// in the markup resolve to these top-level functions. Do not convert to a module.
  ...
  function onInput() {
    ...
    var dates      = getDates();
    var surveyOk   = !qualtricsOn || document.getElementById("surveySelect").value;
  ...
  function renderAll(sigByDate) {
    ...
    var dates      = getDates();
    var surveyBase = qualtricsOn ? document.getElementById("surveySelect").value : null;
    ...
    if (qualtricsOn && surveyBase) {
      for (var j = 0; j < dates.length; j++) {
        var date2    = fmt(dates[j]);
        var display2 = fmtDisplay(dates[j]);
        var svUrl    = surveyBase
    ...
    document.getElementById("sectionTitle").textContent =
      "Class "+key+" - "+total+" QR Code"+(total>1?"s":"");
  ...
  function buildCard(gridId, qrId, dayIdx, key, sname, cLabel, dateDisplay, dateParam, url) {
    ...
    meta.innerHTML =
      "<div class='qr-meta-row'><span class='qr-meta-label'>Key</span><span class='qr-meta-val'>Class "+esc(key)+"</span></div>"+
